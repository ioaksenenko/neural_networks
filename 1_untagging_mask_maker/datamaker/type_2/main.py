...
def scq(sample):
    ...
    patterns = [[r'</p>', False], [r'<b>', True], [r'</b>', False], [r'\d\.\s', True], [r';', False]]
...
def tiq(sample):
    fragments = list(filter(None, re.split(r'<p>', sample)))
    patterns = [[r'</p>', False], [r'<b>\[', True], [r'\]</b>', False]]
    parts = re.findall(r'<p>', sample)
    res = processing(fragments, patterns)
    res = ''.join([parts[i] + res[i] for i in range(len(res))])
    return res
...
def niq(sample):
    fragments = list(filter(None, re.split(r'<p>', sample)))
    patterns = [[r'</p>', False], [r'<i>\[', True], [r'\]</i>', False]]
    parts = re.findall(r'<p>', sample)
    res = processing(fragments, patterns)
    res = ''.join([parts[i] + res[i] for i in range(len(res))])
    return res
...
def mq(sample):
    fragments = list(filter(None, re.split(r'<p>', sample)))
    patterns = [[r'</p>', False], [r'Ответ:\s', True], [r'<i>', True], [r'(</i>\s\–\s|</i>,\s|</i>)', False]]
    parts = re.findall(r'<p>', sample)
    res = processing(fragments, patterns)
    res = ''.join([parts[i] + res[i] for i in range(len(res))])
    return res
...
def oq(sample):
    fragments = list(filter(None, re.split(r'<p>', sample)))
    patterns = [[r'</p>', False], [r'Ответ:\s<i>', True], [r'</i>,\s<i>', False], [r'</i>', False]]
    parts = re.findall(r'<p>', sample)
    res = processing(fragments, patterns)
    res = ''.join([parts[i] + res[i] for i in range(len(res))])
    return res
...
def cq(sample):
    fragments = list(filter(None, re.split(r'<p>', sample)))
    patterns = [[r'</p>', False], [r'<b>\[', True], [r'\]</b>', False], [r'<i>\[', True], [r'\]</i>', False], [r'<b>', True], [r'</b>', False], [r'\d\.\s', True], [r';', False]]
    parts = re.findall(r'<p>', sample)
    res = processing(fragments, patterns)
    res = ''.join([parts[i] + res[i] for i in range(len(res))])
    return res
...
def test():
    scq_sample = "<p>Укажите административный центр Томской области:</p><p>1. Асино;</p><p>2. Северск;</p><p>3. Стрежевой;</p><p><b>4. Томск;</b></p>"
    mcq_sample = "<p>Укажите города Томской области:</p><p><b>1. Томск;</b></p><p>2. Кожевниково;</p><p><b>3. Северск;</b></p><p><b>4. Асино;</b></p><p>5. Юрга;</p>"
    tiq_sample = "<p>Какой город является административным центром Томской области?</p><p><b>[Томск]</b></p>"
    niq_sample = "<p>Укажите численность населения г. Томска в 2016 году. Ответ укажите в тыс. чел.</p><p><i>[569 293]</i></p>"
    mq_sample = "<p>Сопоставьте населённые пункты Томской области и год их основания.</p><p>Ответ: <i>Томск</i> – <i>1604</i>, <i>Северск</i> – <i>1949</i>, <i>Каргасок</i> – <i>1640</i>, <i>Кривошеино</i> – <i>1826</i></p>"
    oq_sample = "<p>Расставьте населённые пункты Томской области по порядку согласно году их основания.</p><p>Ответ: <i>Томск</i>, <i>Каргасок</i>, <i>Кривошеино</i>, <i>Северск</i></p>"
    cq_sample = "<p>Укажите административный центр Томской области:</p><p>1. Асино;</p><p>2. Северск;</p><p>3. Стрежевой;</p><p><b>4. Томск;</b></p><p>Он был основан в <i>[1604]</i> году.</p>"
