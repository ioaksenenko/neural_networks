...
class DataPreparation:
    ...
    def __init__(self,
        ...
        self.train_input_shape = np.shape(self.train_input_sequences)
        self.train_output_shape = np.shape(self.train_output_sequences)
        self.valid_input_shape = np.shape(self.valid_input_sequences)
        self.valid_output_shape = np.shape(self.valid_output_sequences)
        self.test_input_shape = np.shape(self.test_input_sequences)
        self.test_output_shape = np.shape(self.test_output_sequences)
        ...
        if self.is_out_sub_seq:
            ...
            self.out_sub_seq_code_len = math.ceil(math.log2(self.output_seq_max_len))
            self.out_sub_seq_codes = [
                [int(element) for element in tuple] for tuple in itertools.product('01', repeat=self.out_sub_seq_code_len)
            ]
        ...
        if self.add_identification_data:
            if self.train_id_file_path is not None:
                ...
                self.train_input_identification_vectors = DataPreparation.repeat_headers(
            ...
            if self.valid_id_file_path is not None:
                ...
                self.valid_input_identification_vectors = DataPreparation.repeat_headers(
            ...
            if self.test_id_file_path is not None:
                ...
                self.test_input_identification_vectors = DataPreparation.repeat_headers(
