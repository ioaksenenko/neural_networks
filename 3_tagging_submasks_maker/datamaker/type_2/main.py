...
class Datamaker:
        ...
        for i in range(n):
            ...
            for j in range(n):
                ...
                for k in range(len(fragment)):
                    input[0] += '_'
                    input[1] += '_' if not (fragment[k] == '<' and (fragment[k+1] == 'b' or fragment[k+2] == 'b') or fragment[k] == '>' and fragment[k-1] == 'b' or fragment[k] == '/' and fragment[k+1] == 'b' or fragment[k] in ['b', 'T']) else fragment[k]
                    ...
                    for l in range(n):
                        if l == j:
                            input[l + 4] += '_' if not (fragment[k] == '<' and (fragment[k+1] == 'b' or fragment[k+2] == 'b') or fragment[k] == '>' and fragment[k-1] == 'b' or fragment[k] == '/' and fragment[k+1] == 'b' or fragment[k] in ['b', 'T']) else fragment[k]
                        else:
                            input[l + 4] += '_'
            ...
            for j in range(len(product)):
                ...
                for k in range(len(fragment)):
                    ...
                    for l in range(len(product)):
                        if l == j:
                            input[2 * l + 3] += '_' if not (fragment[k] == '<' and (fragment[k+1] == 'b' or fragment[k+2] == 'b') or fragment[k] == '>' and fragment[k-1] == 'b' or fragment[k] == '/' and fragment[k+1] == 'b' or fragment[k] in ['b', 'T']) else fragment[k]
                            for t in range(len(product)):
                                if product[t] == product[j]:
                                    input[2 * t + 4] += '_' if not (fragment[k] == '<' and (fragment[k+1] == 'b' or fragment[k+2] == 'b') or fragment[k] == '>' and fragment[k-1] == 'b' or fragment[k] == '/' and fragment[k+1] == 'b' or fragment[k] in ['b', 'T']) else fragment[k]
                                else:
                                    input[2 * t + 4] += '_'
                        else:
                            input[2 * l + 3] += '_'
                input[2] += '___' + ('___' if product[j] == 'T' else '') + str(j + 1) + '._;' + ('____' if product[j] == 'T' else '') + '____'
            if len(re.findall(r'<b>', input[1])) > 1:
                inputs.append(input)
                outputs.append(output)
        return inputs, outputs
    ...
    def text_input_question_generate(n=1, m=1):
        ...
        for i in range(n):
            ...
            output.append('T')
            input[0] += '_________________' + '________'
            for j in range(n):
                if j == i:
                    input[2 * j + 1] += '___<b>[T]</b>____' + '________'
                    input[2 * j + 2] += '_________________' + '___T____'
                else:
                    input[2 * j + 1] += '_________________' + '________'
                    input[2 * j + 2] += '_________________' + '________'
        inputs.append(input)
        outputs.append(output)
    ...
    def number_input_question_generate(n=1, m=1):
        ...
        for i in range(n):
            ...
            for j in range(n):
                if j == i:
                    input[2 * j + 1] += '___<i>[T]</i>____' + '________'
                    input[2 * j + 2] += '_________________' + '___T____'
                else:
                    input[2 * j + 1] += '_________________' + '________'
                    input[2 * j + 2] += '_________________' + '________'
    ...
    def matching_question_generate(n=1, m=1):
        inputs = []
        outputs = []
        input = ['___T_____________', '_________________']
        output = ['[item]T[/item]', '[matching][/matching]']
        for _ in range(n):
            input.append('_________________')
        for i in range(n):
            output.append('[subitem]T[answer]T[/answer][/subitem]')
            input[0] += '_________________' + ('_' if i != n - 1 else '')
            input[1] += '<i>T</i>–<i>T</i>' + (',' if i != n - 1 else '')
            for j in range(n):
                if i == j:
                    input[j + 2] += '<i>T</i>–<i>T</i>' + ('_' if i != n - 1 else '')
                else:
                    input[j + 2] += '_________________' + ('_' if i != n - 1 else '')
    ...
    def ordering_question_generate(n=1, m=1):
        ...
        outputs = []
        input = ['___T_____________', '_________________']
        output = ['[item]T[/item]', '[ordering][/ordering]']
        for _ in range(n):
            input.append('_________________')
        for i in range(n):
            output.append('[subitem]T[/subitem]')
            input[0] += '________' + ('_' if i != n - 1 else '')
            input[1] += '<i>T</i>' + (',' if i != n - 1 else '')
            for j in range(n):
                if i == j:
                    input[j + 2] += '<i>T</i>' + ('_' if i != n - 1 else '')
                else:
                    input[j + 2] += '________' + ('_' if i != n - 1 else '')
        for i in range(len(input)):
            input[i] += '____'
        inputs.append(input)
        outputs.append(output)
