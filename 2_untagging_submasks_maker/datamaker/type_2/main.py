...
class Datamaker:
    ...
    def single_choice_question_generate(n=1, m=1):
        ...
        for i in range(n):
            ...
            for j in range(n):
                fragment = '<p>' + ('<b>' if i == j else '') + str(j+1) + '.T;' + ('</b>' if i == j else '') + '</p>'
                for k in range(len(fragment)):
                    output[0] += '_'
                    output[1] += '_' if not (fragment[k] == '<' and (fragment[k+1] == 'b' or fragment[k+2] == 'b') or fragment[k] == '>' and fragment[k-1] == 'b' or fragment[k] == '/' and fragment[k+1] == 'b' or fragment[k] in ['b', 'T']) else fragment[k]
                    ...
                    for l in range(n):
                        if l == j:
                            output[l + 4] += '_' if not (fragment[k] == '<' and (fragment[k+1] == 'b' or fragment[k+2] == 'b') or fragment[k] == '>' and fragment[k-1] == 'b' or fragment[k] == '/' and fragment[k+1] == 'b' or fragment[k] in ['b', 'T']) else fragment[k]
                        else:
                            output[l + 4] += '_'
    ...
    def multi_choice_question_generate(n=1, m=1):
        ...
        for product in products:
            ...
            for j in range(len(product)):
                fragment = '<p>' + ('<b>' if product[j] == 'T' else '') + str(j + 1) + '.T;' + ('</b>' if product[j] == 'T' else '') + '</p>'
                for k in range(len(fragment)):
                    output[0] += '_'
                    output[1] += '_' if not (fragment[k] == '<' and (fragment[k+1] == 'b' or fragment[k+2] == 'b') or fragment[k] == '>' and fragment[k-1] == 'b' or fragment[k] == '/' and fragment[k+1] == 'b' or fragment[k] in ['b', 'T']) else fragment[k]
                    for l in range(len(product)):
                        if l == j:
                            output[2 * l + 3] += '_' if not (fragment[k] == '<' and (fragment[k+1] == 'b' or fragment[k+2] == 'b') or fragment[k] == '>' and fragment[k-1] == 'b' or fragment[k] == '/' and fragment[k+1] == 'b' or fragment[k] in ['b', 'T']) else fragment[k]
                            for t in range(len(product)):
                                if product[t] == product[j]:
                                    output[2 * t + 4] += '_' if not (fragment[k] == '<' and (fragment[k+1] == 'b' or fragment[k+2] == 'b') or fragment[k] == '>' and fragment[k-1] == 'b' or fragment[k] == '/' and fragment[k+1] == 'b' or fragment[k] in ['b', 'T']) else fragment[k]
                                else:
                                    output[2 * t + 4] += '_'
                        else:
                            output[2 * l + 3] += '_'
                output[2] += '___' + ('___' if product[j] == 'T' else '') + str(j + 1) + '._;' + ('____' if product[j] == 'T' else '') + '____'
                input += fragment
            if len(re.findall(r'<b>', input)) > 1:
                inputs.append(input)
                outputs.append(output)
        return inputs, outputs
    ...
    def text_input_question_generate(n=1, m=1):
        ...
        for _ in range(2 * n):
            output.append('________')
        for i in range(n):
            input += '<p><b>[T]</b></p>' + '<p>T</p>'
            output[0] += '_________________' + '________'
            for j in range(n):
                if j == i:
                    output[2 * j + 1] += '___<b>[T]</b>____' + '________'
                    output[2 * j + 2] += '_________________' + '___T____'
                else:
                    output[2 * j + 1] += '_________________' + '________'
                    output[2 * j + 2] += '_________________' + '________'
        inputs.append(input)
        outputs.append(output)
    ...
    def number_input_question_generate(n=1, m=1):
        ...
        for _ in range(2 * n):
            output.append('________')
        for i in range(n):
            input += '<p><i>[T]</i></p>' + '<p>T</p>'
            output[0] += '_________________' + '________'
            for j in range(n):
                if j == i:
                    output[2 * j + 1] += '___<i>[T]</i>____' + '________'
                    output[2 * j + 2] += '_________________' + '___T____'
                else:
                    output[2 * j + 1] += '_________________' + '________'
                    output[2 * j + 2] += '_________________' + '________'
    ...
    def matching_question_generate(n=1, m=1):
        ...
        outputs = []
        input = '<p>T</p><p>Ответ:'
        output = ['___T_____________', '_________________']
        for _ in range(n):
            output.append('_________________')
        for i in range(n):
            input += '<i>T</i>–<i>T</i>' + (',' if i != n - 1 else '')
            output[0] += '_________________' + ('_' if i != n - 1 else '')
            output[1] += '<i>T</i>–<i>T</i>' + (',' if i != n - 1 else '')
            for j in range(n):
                if i == j:
                    output[j + 2] += '<i>T</i>–<i>T</i>' + ('_' if i != n - 1 else '')
                else:
                    output[j + 2] += '_________________' + ('_' if i != n - 1 else '')
        input += '</p>'
    ...
    def ordering_question_generate(n=1, m=1):
        ...
        for _ in range(n):
            output.append('_________________')
        for i in range(n):
            input += '<i>T</i>' + (',' if i != n - 1 else '')
            output[0] += '________' + ('_' if i != n - 1 else '')
            output[1] += '<i>T</i>' + (',' if i != n - 1 else '')
            for j in range(n):
                if i == j:
                    output[j + 2] += '<i>T</i>' + ('_' if i != n - 1 else '')
                else:
                    output[j + 2] += '________' + ('_' if i != n - 1 else '')
        input += '</p>'
        for i in range(len(output)):
            output[i] += '____'
        inputs.append(input)
    ...
    def make_test_dataset(file_name):
        ...
        for i in range(len(types)):
            if re.match(r'Вопрос\s+1\.\s*', types[i]):
                ...
                Datamaker.select_sample(r'–', masks[i], 'MQ', input_file, output_file)
                pass
            elif re.match(r'Вопрос\s+6\.\s*', types[i]):
                Datamaker.select_sample(r'<i>T</i>', masks[i], 'OQ', input_file, output_file)
                pass
        input_file.close()
        output_file.close()
...
if __name__ == "__main__":
    """
    inputs, outputs = Datamaker.number_input_question_generate(3)
    for i in range(len(inputs)):
        print(inputs[i])
