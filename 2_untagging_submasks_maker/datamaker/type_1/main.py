...
class Datamaker:
        ...
        for i in range(n):
            ...
            for j in range(n):
                ...
                for k in range(len(fragment)):
                    output[0] += '_'
                    output[1] += '_' if not (fragment[k] in ['~', '='] or fragment[k] == 'T' and fragment[k-1] in ['~', '=']) else fragment[k]
                    output[3] += '_' if not (fragment[k] in ['<', '/', 'p', '>']) else fragment[k]
                    for l in range(n):
                        if l == j:
                            output[l + 4] += '_' if not (fragment[k] in ['~', '='] or fragment[k] == 'T' and fragment[k - 1] in ['~', '=']) else fragment[k]
                        else:
                            output[l + 4] += '_'
                output[2] += '___' + str(j+1) + '.' + '__;____'
                input += fragment
            inputs.append(input)
            ...
            for j in range(len(product)):
                ...
                for k in range(len(fragment)):
                    ...
                    for l in range(len(product)):
                        if l == j:
                            output[2 * l + 3] += '_' if not (fragment[k] in ['~', '='] or fragment[k] == 'T' and fragment[k - 1] in ['~', '=']) else fragment[k]
                            for t in range(len(product)):
                                if product[t] == product[j]:
                                    output[2 * t + 4] += '_' if not (fragment[k] in ['~', '='] or fragment[k] == 'T' and fragment[k - 1] in ['~', '=']) else fragment[k]
                                else:
                                    output[2 * t + 4] += '_'
                        else:
                            output[2 * l + 3] += '_'
    ...
    def text_input_question_generate(n=1):
        ...
        for i in range(n):
            input += '<p>{=T}</p>' + '<p>T</p>'
            output[0] += '___________' + '________'
            for j in range(n):
                if j == i:
                    output[2 * j + 1] += '___{=T}____' + '________'
                    output[2 * j + 2] += '___________' + '___T____'
                else:
                    output[2 * j + 1] += '___________' + '________'
                    output[2 * j + 2] += '___________' + '________'
        inputs.append(input)
        outputs.append(output)
        ...
        for permutation in permutations:
            ...
            for j in range(len(permutation)):
                ...
                for k in range(len(fragment)):
                    output[0] += '_'
                    output[1] += '_'
                    for l in range(3, len(output)):
                        output[l] += '_'
                output[2] += '___' + str(j + 1) + ')_____'
            for j in range(len(permutation)):
                input += '<p>' + iterable[j] + ')T</p>'
    ...
    def make_test_dataset(file_name):
        ...
        for i in range(len(types)):
            if re.match(r'Вопрос\s+1\.\s*', types[i]):
                ...
                Datamaker.select_sample(r'\d+[,<]', masks[i], 'OQ', input_file, output_file)
                pass
        input_file.close()
        output_file.close()
...
if __name__ == "__main__":
    """
    inputs, outputs = Datamaker.number_input_question_generate(2)
    for i in range(len(inputs)):
        print(inputs[i])
