...
class Datamaker:
    ...
    def single_choice_question_generate(n=1, is_item=True):
        ...
        for i in range(n):
            ...
            for j in range(n):
                ...
                output += ['4', '3', '3', '2,' + str(j+5), '3', '4'] if j < 3 else ['4', '0', '0', '2,' + str(j+5), '0', '4']
            input += ['<p>', '_', '</p>']
            output += ['4', '1', '4']
            inputs.append(input)
    ...
    def multi_choice_question_generate(n=1, is_item=True):
        ...
        for product in products:
            ...
            for j in range(len(product)):
                ...
                output += ['0', '3', '3', element, '3', '0'] if j < 3 else ['4', '0', '0', '2,' + str(j + 5), '0', '4']
            input += ['<p>', '_', '</p>']
            output += ['0', '1', '0']
            inputs.append(input)
            outputs.append(output)
    ...
    def matching_question_generate(n=1, is_item=True):
        ...
        for permutation in permutations:
            ...
            for j in range(len(permutation)):
                input += ['<p>', str(j + 1), ')', '_', '</p>']
                output += ['0', '3', '3', '3', '0'] if j < 3 else ['0', '0', '0', '0', '0']
            for j in range(len(permutation)):
    ...
    def write_to_files(inputs, outputs, label):
        ...
        for i in range(len(inputs)):
            ...
            output_file.write(str(outputs[i]) + '\n')
        input_file.close()
        output_file.close()
            ...
            if re.match(r'Вопрос\s+1\.\s*', types[i]):
                ...
                for j in range(len(inputs)):
                    ...
                    if lhs == rhs:
                        ...
                        input_file.write(str(inputs[j]) + '\n')
                        output_file.write('Вопрос 1.\n')
                        output_file.write(str(outputs[j]) + '\n')
